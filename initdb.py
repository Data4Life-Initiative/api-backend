...
for disease in DISEASES:
    if Disease.objects.filter(name__iexact=disease["name"]).count() == 0:
        ...
        disease = Disease.objects.create(name=disease["name"])
        for infection_status in disease["infection_status"]:
            DiseaseInfectionStatus.objects.create(disease=disease, infection_status=infection_status)
...
# Todo : Populate risk assessment recommendation model
